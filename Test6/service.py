...
把所有子服务包装成 BentoML API，提供 RESTful API + 静态前端
BentoML 1.4+ 类式服务写法
"""
...
def _make_plotly_json(fig) -> Optional[dict]:
    """把 plotly figure 转为 JSON 可序列化对象"""
...
class SolarChargingDemo:
    ...
    # API: 健康检查
    # ============================================================
    @bentoml.api(route="/api/health_check")
    async def health_check(self, payload: Optional[dict] = None) -> dict:
    ...
    # API: 获取实时气象
    # ============================================================
    @bentoml.api(route="/api/weather")
    async def weather(self, payload: Optional[dict] = None) -> dict:
    ...
    async def predict(self, payload: Optional[dict] = None) -> dict:
        if payload is None:
            payload = {}
    ...
    # API: 仅光伏预测
    # ============================================================
    @bentoml.api(route="/api/solar_predict")
    async def solar_predict(self, payload: Optional[dict] = None) -> dict:
        if payload is None:
            payload = {}
    ...
    # API: 仅充电预测
    # ============================================================
    @bentoml.api(route="/api/charging_predict")
    async def charging_predict(self, payload: Optional[dict] = None) -> dict:
        if payload is None:
            payload = {}
        n_steps = payload.get("n_steps", 96)
    ...
    # ============================================================
    @bentoml.api(route="/api/plot_prediction")
    async def plot_prediction(self, payload: Optional[dict] = None) -> dict:
        if payload is None:
            payload = {}
        n_steps = payload.get("n_steps", 96)
        ...
        return _make_plotly_json(fig) or {}

    # ============================================================
    # API: 生成天气图表
    # ============================================================
    @bentoml.api(route="/api/plot_weather")
    async def plot_weather(self, payload: Optional[dict] = None) -> dict:
        from weather_service import fetch_weather_data
        data = fetch_weather_data()
        if data is None:
            return {"error": "无气象数据"}
    ...
    # API: 电价曲线
    # ============================================================
    @bentoml.api(route="/api/price_curve")
    async def price_curve(self, payload: Optional[dict] = None) -> dict:
    ...
    # API: 历史数据
    # ============================================================
    @bentoml.api(route="/api/historical")
    async def historical(self, payload: Optional[dict] = None) -> dict:
    ...
    # 模型状态
    # ============================================================
    @bentoml.api(route="/api/model_status")
    async def model_status(self, payload: Optional[dict] = None) -> dict:
...
# 挂载到 BentoML 服务 — svc 即 SolarChargingDemo 类 (被 @bentoml.service 装饰后即 Service 实例)
