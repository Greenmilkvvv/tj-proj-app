...
def create_app():
    ...
    # 合并 CSS (不通过 gr.Blocks(css=...) 传递，以规避 Gradio 6.10 body_css=None 的 Jinja2 模板异常)
    ...
    with gr.Blocks(
        ...
        # 内联注入 CSS (规避 gradio 6.10 中 body_css=None → Jinja2 UndefinedError)
        gr.HTML(f"<style>{combined_css}</style>")
        # 标题
        gr.HTML(
            f"""
